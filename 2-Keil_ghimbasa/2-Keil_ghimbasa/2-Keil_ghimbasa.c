...
int main(void) {
  ...
  uint8_t dataReady;
	...
	PortK0K3_Init();
	int count = 0;
	int slices = 1; // CHANGE THIS VARIABLE TO CHANGE HOW MANY SLICES OF MEASUREMENTS TO TAKE
	
	while(1){//keep checking if the button is pressed 	
		if(((GPIO_PORTM_DATA_R&0b00000001)==0&&(GPIO_PORTE_DATA_R&0b000000001)==0)){ // When button is pressed
		
			UART_printf("Running Program...\r\n");
			sprintf(printf_buffer,"2DX4 Final Project - Alex Ghimbasan - 400203560 - ghimbasa \r\n");
			...
			for (int z = 0; z < slices; z++){ // Loop ing through desired amount of slices
				
				//Turning the motor using full step method (2048 steps per rotation)
				while(count<2048){
					count++;
					GPIO_PORTK_DATA_R = 0b00001100; // IN4 & IN3 ON
					SysTick_Wait10ms(5);
					
					count++;
					GPIO_PORTK_DATA_R = 0b00000110; // IN3 & IN2 ON
					SysTick_Wait10ms(5);
					
					count++;
					GPIO_PORTK_DATA_R = 0b00000011; // IN2 & IN1 ON
					SysTick_Wait10ms(5);
					
					count++;
					GPIO_PORTK_DATA_R = 0b00001001; // IN1 & IN4 ON
					SysTick_Wait10ms(5);
					
					// Measuring distance with the sensor (this happens 512 times per 360 degree rotation)
					while (dataReady == 0){
				...
				// 10 second delay to allow forward movement of 20cm
				// LED1 blinks every second to give user sense of time
				for (int k = 0; k < 10; k++){
			...
			SysTick_Wait10ms(20); // delay before checking button press again
			count = 0;
				
		} // End of if for button press
	} // End of while loop for button check
}
